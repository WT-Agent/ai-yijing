  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">核心卦象</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">哲学启示</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">易经周易哲学与理性思维指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 易经周易哲学与卦象专家');
...
const metricsList = [
  {
    "key": "hexagramInterpretationPrecision",
    "label": "卦辞爻辞解读精准度"
  },
  {
    "key": "yijingPhilosophyWisdom",
    "label": "周易哲学启发深度"
  },
  {
    "key": "decisionMakingGuidance",
    "label": "决策启示实用性"
  },
  {
    "key": "traditionalCultureElegance",
    "label": "国学文化雅致分"
  },
  {
    "key": "rationalPositiveMindset",
    "label": "理性积极心态引导"
  }
];
const guideRules = [
  {
    "title": "以哲学智慧引导理性决策",
    "advice": "强调“易为君子谋”，把易经作为辩证思维与大局观工具，引导用户积极面对人生",
    "avoid": "切忌进行宿命论迷信宣扬"
  },
  {
    "title": "遵循“物极必反”变通规律",
    "advice": "用阴阳消长原理解释顺境与逆境的辩证转化，鼓励用户在顺境中居安思危，在逆境中积蓄力量",
    "avoid": "避免用僵化绝对的视角看待事物"
  },
  {
    "title": "提炼经典警句修身养性",
    "advice": "引用“君子终日乾乾”、“厚德载物”等国学经典，提升文化内涵与心灵抚慰",
    "avoid": "避免大段生涩难懂的古文而不做通俗解读"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[YIJING_SCORES\][\s\S]*?\[\/YIJING_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[YIJING_SCORES\](.*?)\[\/YIJING_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('yijing_history_records');
...
const saveHistory = () => {
  localStorage.setItem('yijing_history_records', JSON.stringify(historyList.value));
};
